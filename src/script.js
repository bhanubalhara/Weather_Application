...
async function getWeather() {
  ...
  if(weather) {
    let formattedCity = weather.name.toLowerCase();
    recentCities =recentCities.filter(city => city.toLowerCase() !== formattedCity);
    recentCities.unshift(weather.name);
    recentCities = recentCities.slice(0,5); //Keeping only 5 recent searched cities
...
function displayWeather(data) {
  ...
  `<div class="flex flex-col items-center text-center bg-white opacity-40 shadow-2xl p-4 rounded-lg">
...
function displayForecast(data) {
  ...
  dailyForecasts.forEach((day) => {
    ...
    forecastDiv.innerHTML += 
    `<div class="bg-white bg-opacity-35 text-black p-4 rounded-lg text-center shadow-2xl flex flex-col items-center">
    <p>
...
// Weather featching based on longitude and latitude
...
// Weather forecasting based on longitude and latitude
